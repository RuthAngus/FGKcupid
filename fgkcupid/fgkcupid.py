...
def plot_2d_prediction(theta, x, y, xerr, yerr, myx, myz, mu, v, xaxis):
    ...
    plt.legend(loc="best")
    plt.savefig("{0}_vs_{1}".format(xlabel, ylabel))


def age_model(mybv, myperiod, plot=False):
    """
    ...
    if plot:
        ...
        plot_2d_prediction(theta, period[m], age[m], period_err[m],
                           age_err[m], myperiod, mybv, mu, v, "period")

        m = (myperiod - 10 < period) * (period < myperiod + 10)
        plot_2d_prediction(theta, bv[m], age[m], bv_err[m], age_err[m], mybv,
                           myperiod, mu, v, "bv")
